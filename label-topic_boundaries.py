...
class TopicBoundaryLabeler:
    ...
    def run_labeling_session(self):
        ...
        for meeting_id, transcript in meetings:
            ...
            topics = self.segmenter.extract_topic_segments(transcript)
            
            for topic in topics:
                char_index = len(" ".join(transcript.split()[:topic['start_index']]))
                context = self.display_boundary_context(transcript, char_index)
                print(context)
